...
Usuario::Usuario(string nombreCompleto, string telefono, int edad, string numeroCuenta, int saldo) {
		this->nombreCompleto=nombreCompleto;
		this->telefono=telefono;
		this->edad=edad;
		this->cuentaB = new CuentaBancaria(numeroCuenta,saldo);
}
...
Usuario::Usuario(const Usuario &usuario) {
	...
	this->cuentaB = new CuentaBancaria(*usuario.cuentaB);
}
...
float Usuario::sacarDinero(float dinero) {
	return cuentaB->sacarDinero(dinero);
}
